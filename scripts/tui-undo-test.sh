...
# autosave persists the reversion. The host pushes noundofile + undoreload=0.
set -uo pipefail
SOCK=rvundo
...
# 1. The user edits, and a view round trip autosaves the edit to disk.
keys Tab; sleep 0.4
...
# 3. Undo after the reload must not revert the agent's work (the reload cleared history;
#    there is no cross-session undofile past to fall back to).
keys Tab; sleep 0.4
keys u; sleep 0.8
...
echo "ok 1 - undo cannot reach past the reload of the agent's change"
...
# 5. In-session undo/redo still work on the user's own edits.
...
keys u; sleep 0.5
wait_gone "XYZQ"
echo "ok 3 - the user's own edits undo and redo normally"
