...
impl App {
    ...
    /// The open file's view-matching comments paired with the visible row each one anchors to,
    /// sorted by row — one entry per comment (its first covered line), so a walk steps comment
    /// by comment rather than line by line through a multi-line anchor. Empty with no open file.
    fn open_file_comment_rows(&self) -> Vec<(usize, usize)> {
        let Some(file) = self.diff_path.as_deref() else {
            return Vec::new();
        };
        let mut out: Vec<(usize, usize)> = self
            ...
            .iter()
            .enumerate()
            .filter(|(_, c)| c.file == file && self.comment_matches_current(c))
            .filter_map(|(ci, c)| {
                let row = self.visible.iter().position(|r| {
                    let no = match c.side {
                        Side::New => r.new_no(),
                        Side::Old => r.old_no(),
                    };
    ...
    /// A file's view-matching comments as store indices, ordered by anchor line — so the walk can
    /// land on that file's first (entering forward) or last (entering backward) comment.
    fn file_comment_indices(&self, file: &str) -> Vec<usize> {
        let mut v: Vec<usize> = self
            ...
            .map(|(ci, _)| ci)
            .collect();
        v.sort_by_key(|&ci| self.store.get(ci).map_or(0, |c| c.start));
        v
    }

    /// Walk the review's comments *across* files, moving the editor and the file-list selection.
    /// Forward (`dir >= 0`) steps to the next commented line in the open file; past its last
    /// comment it advances to the next file — in file-list order — that carries a view-matching
    /// comment and lands on that file's first comment. Backward is the mirror. Both wrap through
    /// the whole changeset (the open file is visited last, so a lone commented file wraps onto
    /// its own first/last comment). With no view-matching comment anywhere it only reports so.
    ///
    /// This is the header button's action. `jump_comment` (the built-in pane's `n`/`N`) wraps
    /// within one file; `walk_comment` crosses files, and drives the editor through
    /// `jump_to_comment` so the embedded-nvim cursor, the diff selection, and the file list all
    /// follow — the same machinery a click in the comments list uses.
    pub fn walk_comment(&mut self, dir: isize) {
        if !self.store.iter().any(|c| self.comment_matches_current(c)) {
            self.status = "no comments to walk".to_string();
            return;
        }
        // Within the open file: the next/prev comment strictly past the cursor.
        let rows = self.open_file_comment_rows();
        let cur = self.diff_cursor;
        let within = if dir >= 0 {
            rows.iter().find(|&&(row, _)| row > cur).map(|&(_, ci)| ci)
        } else {
            rows.iter().rev().find(|&&(row, _)| row < cur).map(|&(_, ci)| ci)
        };
        if let Some(ci) = within {
            self.jump_to_comment(ci);
            return;
        }
        // Past the open file's comments: the next/prev file that carries one. The scan wraps
        // through every file and ends back on the open one, so a single commented file wraps
        // onto its own first (forward) or last (backward) comment.
        let order: Vec<String> = self.entries.iter().map(|e| e.path.clone()).collect();
        let n = order.len();
        if n == 0 {
            return;
        }
        let here = order.iter().position(|p| Some(p.as_str()) == self.diff_path.as_deref());
        let base = here.unwrap_or(if dir >= 0 { n - 1 } else { 0 });
        for step in 1..=n {
            let idx = if dir >= 0 { (base + step) % n } else { (base + n - (step % n)) % n };
            let comments = self.file_comment_indices(&order[idx]);
            let pick = if dir >= 0 { comments.first() } else { comments.last() };
            if let Some(&ci) = pick {
                self.jump_to_comment(ci);
                return;
            }
