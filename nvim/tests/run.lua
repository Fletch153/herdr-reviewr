...
cr_map.callback()
check(
  "Enter past the last hunk reports nav next with its file",
  #navs == 1 and navs[1].action == "nav" and navs[1].payload.dir == "next" and navs[1].payload.file == "w.txt",
  vim.inspect(navs)
)
bs_map.callback()
...
check(
  "the last hunk's revert hands the walk onward with its file",
  #rnavs == 1 and rnavs[1].action == "nav" and rnavs[1].payload.dir == "next" and rnavs[1].payload.file == "r.txt",
  vim.inspect(rnavs)
)
vim.api.nvim_win_set_cursor(0, { 1, 0 })
